...
printMan() {
    ...
    printf "Supported networks: main-ethereum, main-op, main-base, demo-ethereum, demo-op, demo-base, staging-bnb, staging-ethereum, staging-arbitrum, staging-avalanche, staging-base, staging-hyperevm, staging-flare, prod-ethereum, prod-optimism, prod-base, prod-polygon, prod-arbitrum, prod-avalanche, prod-bnb, prod-unichain, prod-berachain, prod-sonic, prod-gnosis, prod-worldchain, prod-hyperliquid, prod-flare\n"
}
...
setup_chain_config() {
    if ! validate_chain_name "$CHAIN_NAME"; then
        ...
        printf "Supported chains: main-ethereum, main-op, main-base, demo-ethereum, demo-op, demo-base, staging-bnb, staging-ethereum, staging-arbitrum, staging-avalanche, staging-base, staging-hyperevm, staging-flare, prod-ethereum, prod-optimism, prod-base, prod-polygon, prod-arbitrum, prod-avalanche, prod-bnb, prod-unichain, prod-berachain, prod-sonic, prod-gnosis, prod-worldchain, prod-hyperliquid, prod-flare\n"
        exit 1
    fi
...
if [ $proceed = "y" ]; then
    ...
    {   
        ...
        if [ "$SHOULD_VERIFY" = true ]; then
            forge script DeployNexus $ENVIRONMENT_NAME $DEFAULT_VALIDATOR --sig "runDeploy(string,address)" --rpc-url $CHAIN_RPC_URL --etherscan-api-key $CHAIN_NAME --private-key $PRIVATE_KEY $VERIFY -vv --broadcast --slow $GAS_SUFFIX 1> ./logs/$CHAIN_NAME/$CHAIN_NAME-deploy-nexus.log 2> ./logs/$CHAIN_NAME/$CHAIN_NAME-deploy-nexus-errors.log
        else
            forge script DeployNexus $ENVIRONMENT_NAME $DEFAULT_VALIDATOR --sig "runDeploy(string,address)" --rpc-url $CHAIN_RPC_URL --private-key $PRIVATE_KEY -vv --broadcast --slow $GAS_SUFFIX 1> ./logs/$CHAIN_NAME/$CHAIN_NAME-deploy-nexus.log 2> ./logs/$CHAIN_NAME/$CHAIN_NAME-deploy-nexus-errors.log
        fi
    } || {
